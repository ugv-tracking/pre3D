...
class ProposalTargetOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        gt_confs  = in_data[4].asnumpy()
        im_info   = in_data[5].asnumpy()
        ...
        rois, labels, bbox_targets, bbox_weights, dim_label, angle_label, conf_label = \
            sample_rois(all_rois, fg_rois_per_image, rois_per_image, self._num_classes, gt_boxes=gt_boxes, gt_dims=gt_dims, gt_angles=gt_angles, gt_confs=gt_confs)
...
class ProposalTargetProp(mx.operator.CustomOpProp):
    ...
    def list_arguments(self):
        if config.TRAIN.BBOX_3D:
            return ['rois', 'gt_boxes', 'gt_dims', 'gt_angles', 'gt_confs', 'im_info']
        else:
            return ['rois', 'gt_boxes', 'im_info']
    ...
    def infer_shape(self, in_shape):
        ...
        if config.TRAIN.BBOX_3D:
            ...
            gt_confs_shape  = in_shape[4]
            im_info_shape   = in_shape[5]
        ...
        if config.TRAIN.BBOX_3D:
            ...
            conf_label_shape  = (self._batch_rois, 1)

        if config.TRAIN.BBOX_3D:
            return [rpn_rois_shape, gt_boxes_shape, gt_dims_shape, gt_angles_shape, gt_confs_shape, im_info_shape], \
