...
def get_vgg_rpn_roi(relu5_3, gt_boxes, orientation, num_classes, num_anchors, config):
	# RPN layers
    ...
    group = mx.symbol.Custom(rois=rois, gt_boxes=gt_boxes_reshape, orientation=orientation_reshape, im_info=im_info, op_type='proposal_3dbox',
...
def get_vgg_3dbox_test(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS)):
	...
	# RCNN network
	group = get_vgg_rpn_roi(relu5_3, gt_boxes, orientation, num_classes, num_anchors, config)
	drop7 = group[0]
	rpn_cls_prob = group[1] 
	rpn_bbox_loss = group[2]
